...
const createError = require("http-errors"),
    ...
    debug = require("debug")("aquasafe20xxapi:server"),
    http = require("http"),
...
//Funny functions
function tokenGen(name: string, pass: string): string {
...
app.post("/users/login", (req: { body: JSON }, res) => {
    ...
    if (password.length === 56 && name !== "")
        userDB.query(
            "SELECT name, pass from users WHERE name in ('" + name + "')",
